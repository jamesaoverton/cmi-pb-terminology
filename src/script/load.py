...
# Grammar used to parse the `structure` column of the `columns` table.
...
class StructureTreeTransformer(Transformer):
    """Transformer to convert a Tree generated by the structure grammar into a list of
    expressions represented as dicts."""
...
def create_schema(config, table_name):
    ...
    for row in columns.values():
        ...
        if structure and not table_name.endswith("_conflict"):
            parser = Lark(structure_grammar)
            tree = parser.parse(structure)
            for expression in StructureTreeTransformer().transform(tree):
